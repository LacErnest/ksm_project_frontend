...
class DescriptionTab extends Component {
    ...
    handleProduitChange (produit){
		this.setState({product:produit,
			descriptionIsDefined: false,
			specification:"",
			description: "",
		})
			
        DescriptionAPI.productProdLang(produit.id,this.props.langue.code).then(res=>{
        if(res.status==200) { 
					var desc =res.data[0]
					console.log("ereryy"+res.data[0].specification)
					this.state.currentDescription= desc;
					this.state.specification= desc.specification;
					this.state.description= desc.description;
	...
	handleSubmit = (e) => {
		...
		data.append('product', this.state.product.name)
							...
							<Col sm={9}>
								...
								<Form onSubmit={this.handleSubmit} onReset={this.handleReset}>
									<Form.Group as={Row} controlId='productsField'>
										...
										<Form.Label column sm={3} ><b>Products</b></Form.Label>
										<Col sm={4}>
...
<br/>
...

									{
										...
										<Container>
											...
											{(this.state.product)?(<h3 className='text-center'>Description non definie pour cette langue.</h3>)
											:(<h3 className='text-center'>Selectionnez un produit.</h3>)}
											</Col>
										</Container>
									}
								...
								{
							(!this.state.descriptionIsDefined && !this.state.newDescription && !this.state.updateDescription&& this.state.product ) &&
								(<>
									<Col className='mg-t-100' align='right'>
													<MDBTooltip placement="bottom">
														<MDBBtn color="deep-orange" 
														 id='ajout'
														onClick={this.handleDefine}>
